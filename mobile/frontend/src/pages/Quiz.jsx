  ...
  const handleSubmit = useCallback(async () => {
    ...
    try {
      ...
      if (!userId) {
        ...
        console.log('No user ID found, creating temporary user');
        const tempUserId = 'temp_user_' + Date.now();
        localStorage.setItem('userId', tempUserId);
        localStorage.setItem('username', 'Utilisateur Temporaire');
        localStorage.setItem('email', 'temp@example.com');
        localStorage.setItem('token', 'temp_token_' + Date.now());

        // Soumettre le quiz avec l'utilisateur temporaire
        const result = await offlineLogic.submitQuiz(id, tempUserId, answers);
        console.log('Submission response with temp user:', result);
        ...
        // Soumettre le quiz avec l'utilisateur connecté
        const result = await offlineLogic.submitQuiz(id, userId, answers);
      ...
      // Rediriger vers la page d'accueil après un délai en cas d'erreur grave
  ...
  useEffect(() => {
    const fetchQuiz = async () => {
      try {
        ...
        // Récupérer le quiz depuis IndexedDB
        const quizData = await offlineLogic.getQuizById(id);
        setQuiz(quizData);
